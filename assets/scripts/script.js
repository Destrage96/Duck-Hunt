$( window ).ready(function () {
    ...
    $( '.wrapper' ).on( 'click', clickScreen );
    $('.wrapper').on( 'click', clickToStart );

    function clickToStart(ev) {
        
        setInterval( function() {
                $('.click_to_start').text('Level ' + level );
        }, 1500 );

        setInterval(function() {
            $( '.click_to_start' ).css({display: 'none'});
        }, 2500 );
    
        setInterval( dog_startAnimation, 2600 );
    }
